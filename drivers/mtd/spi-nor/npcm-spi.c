...
struct npcm7xx_chip {
	u32 fiu_num;	/*TODO: is this even used? */
	u32 clkrate;
	u32 chipselect;
 ...
 */
static int npcm7xx_mtd_ram_register(struct npcm7xx_spi_bus *host)
{
	...
	*mtd = (struct mtd_info) {
	...
	SET_REG_FIELD(FIU_DRD_CFG(host->fiu_num),
		FIU_DRD_CFG_ACCTYPE, 3);
	
	/* set burst to 16byte*/
	SET_REG_FIELD(FIU_DWR_CFG(host->fiu_num),
		FIU_DWR_CFG_W_BURST, 3);
