...
static int npcm_pspi_probe(struct platform_device *pdev)
{
	...
	pdev->id = of_alias_get_id(np, "pspi");
	if (pdev->id < 0)
		pdev->id = 0;
	...
	ret = devm_request_irq(&pdev->dev, irq, npcm_pspi_handler,
			       0, "npcm-pspi", priv);
	if (ret) {
...
MODULE_DESCRIPTION("NPCM PSPI Controller driver");
