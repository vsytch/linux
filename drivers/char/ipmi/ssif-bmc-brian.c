...
{
	u8 i2cctl1;
	//printk("moh2 npcm_i2c_slave_int_enable");
...
{
#if 0
	//printk("moh3 npcm_i2c_slave_enable");
	//printk("moh  enable bus->reg =%x enable=%x \n",bus->reg,enable  );
	//printk("moh  enable  client->addr=%x addr_type=%x \n" ,addr, addr_type );
	unsigned long flags;
	u8 i2cctl1;
	u8 i2cctl3;
	u8 sa_reg;
...
static int receive_ssif_bmc_request(struct ssif_bmc *ssif_bmc, bool non_blocking,
					struct ssif_msg *request)
{
	//printk("moh receive_ssif_bmc_request");
	int res;
	unsigned long flags;
...
static int send_ssif_bmc_response(struct ssif_bmc *ssif_bmc, bool non_blocking,
					struct ssif_msg *response)
{
	//printk("moh send_ssif_bmc_response\n");
	int res;
	unsigned long flags;
	//printk("moh send_ssif_bmc_response non_blocking=%x \n",non_blocking);
	if (!non_blocking) {
try_again:
		res = wait_event_interruptible(ssif_bmc->wait_queue,
					       !ssif_bmc->response_in_progress);
		//printk("moh send_ssif_bmc_response res=%d \n",res);
		if (res)
			return res;
	}
					...
					loff_t *ppos)
{
	...
	//printk("moh ssif_bmc_read ret=%x",ret);
	//printk("moh ssif_bmc_read ret=%x",msg->cmd);
	if (ret < 0)
		goto out;
					...
					loff_t *ppos)
{
	 ...
	 */
	npcm_i2c_slave_enable(ssif_bmc->i2c_bus, 0, ssif_bmc->client->addr, true);
#if 0
	if (IS_ENABLED(CONFIG_ARCH_NPCM7XX))
		{
			printk("moh1 enable slave");
			//printk("moh  enable bus->reg =%x \n",ssif_bmc->i2c_bus2->reg  );
		//printk("moh  enable  client->addr  =%x\n" ,client->addr );
		npcm_i2c_slave_enable(ssif_bmc->i2c_bus2, 0, ssif_bmc->client->addr, true);
		}
	else
...
static int handle_request(struct ssif_bmc *ssif_bmc)
{
	...
	if (IS_ENABLED(CONFIG_ARCH_NPCM7XX))
	{
		...
		//printk("moh  enable bus->reg =%x \n",ssif_bmc->i2c_bus2->reg  );
		printk("moh2 enable  client->addr  =%x\n" ,ssif_bmc->client->addr );
		
		//ssif_bmc->i2c_bus2->slv_rd_size = 0;
		//ssif_bmc->i2c_bus2->slv_wr_size = 0;
		//ssif_bmc->i2c_bus2->slv_rd_ind = 0;
		//ssif_bmc->i2c_bus2->slv_wr_ind = 0;
	
		printk("moh1 disable slave");
		struct npcm_i2c *bus = ssif_bmc->i2c_bus2;
		npcm_i2c_slave_enable(ssif_bmc->i2c_bus2, 0, ssif_bmc->client->addr, false);
				...
				enum i2c_slave_event event, u8 *val)
{
	...
	case I2C_SLAVE_WRITE_RECEIVED:
		...
		if (ssif_bmc->msg_idx == 0) {
			...
			if ((ssif_bmc->msg_idx - 1) >= ssif_msg_len(&ssif_bmc->request)) {
				ssif_bmc->msg_idx = 0; //reset index for read after handle write data
	...
	/*if (val != NULL)
		printk("event:%d, idx: %d, val:%x\n", event, ssif_bmc->msg_idx, *val);*/
