/*
 * npcm750-rng.c - RNG driver for NPCM750 CPU.
 *
...
static int npcm750_rng_data_present(struct hwrng *rng, int wait)
{
	...
	for (i = 0; i < 20; i++) {
		data = (npcm750_rng_read_reg(RNGCS_REG) & RNG_DATA_VALID) ? 1 : 0;
		if (data || !wait)
			break;
...
#ifdef RNG_DEBUG
	mdelay(10);
	printk(KERN_INFO "RNG-Random Number Generator 0x%x\n", npcm750_rng_read_reg(RNGD_REG));
#else
    printk(KERN_INFO "RNG-Random Number Generator\n");
#endif
...
static int npcm750_rng_suspend(struct platform_device *pdev, pm_message_t message)
{   
    u32 val;
