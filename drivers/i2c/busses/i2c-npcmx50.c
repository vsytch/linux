...
/*---------------------------------------------------------------------------------------------------------*/
static inline void SET_REG_FIELD(unsigned char __iomem *mem, bit_field_t bit_field, u8 val) {
    u8 tmp = ioread8(mem);
...
/*---------------------------------------------------------------------------------------------------------*/
static inline u8 READ_REG_FIELD(unsigned char __iomem *mem, bit_field_t bit_field) {
    u8 tmp = ioread8(mem);
...
#define SMBADDR(bus, i)           (bus->base + 0x008 + (u32)(((int)i*4) +\
...
/*---------------------------------------------------------------------------------------------------------*/
static inline void SMB_WriteByte (
...
/*---------------------------------------------------------------------------------------------------------*/
static inline bool SMB_ReadByte (
    nuvoton_i2c_bus_t *bus,
    u8 *      data
)
{
...
/*---------------------------------------------------------------------------------------------------------*/
static inline void SMB_StallAfterStart (
    nuvoton_i2c_bus_t *bus,
    bool      stall
)
{
...
{
    unsigned long flags;
    u8 SmbAddrX_Addr = BUILD_FIELD_VAL(SMBADDRx_ADDR, addr) | BUILD_FIELD_VAL(SMBADDRx_SAEN, enable);
...
/*---------------------------------------------------------------------------------------------------------*/
static bool SMB_StartMasterTransaction (
    nuvoton_i2c_bus_t *bus,
    u8           slave_addr,
    u16          nwrite,
    u16          nread,
...
/*---------------------------------------------------------------------------------------------------------*/
static bool SMB_StartSlaveReceive (
    nuvoton_i2c_bus_t *bus,
    u16          nread,
    u8*          read_data
)
...
/*                  endless interrupt from SDAST for the slave RCV or TX !                                 */
/*---------------------------------------------------------------------------------------------------------*/
static bool SMB_StartSlaveTransmit (
    nuvoton_i2c_bus_t *bus,
    u16          nwrite,
    u8*          write_data
)
    ...
    if (bus->fifo_use == true)
    {
        ...
        if (nwrite > 0)
        {
            ...
            else
            {
                ...
                /*-----------------------------------------------------------------------------------------*/
                smbtxf_ctl = BUILD_FIELD_VAL(SMBTXF_CTL_THR_TXIE, (bool)(bus->threshold_fifo != SMBUS_FIFO_SIZE)) |
...
/* Parameters:                                                                                             */
/*                  bus    - SMB module number                                                      */
/*                  enable        -  bool indicating whether to enable or disable the feature           */
/*                                                                                                         */
/* Returns:         none                                                                                   */
/* Side effects:                                                                                           */
/* Description:                                                                                            */
