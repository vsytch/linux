...
typedef struct
{
	volatile u8          *const pIDR;

	/** Pointer to ODR */
	volatile u8          *const pODR;

	/** Pointer to CMR */
	volatile u8          *const pCMR;
	
	/** Pointer to STR */
	volatile u8          *const pSTR;

	/** Pointer to CTL*/
	volatile u8          *const pCTL;
	
	/** Pointer to IC*/
	volatile u8          *const pIC;
	
	/** Pointer to IE*/
	volatile u8          *const pIE;
} sLPCRegsPtrType;
...
typedef struct
{
	...
	/** Receive buffer */
	u8   au8HIFReceiveBuf[KCS_PACKAGE_MAX];

	/** Transmit buffer */
	u8   au8HIFTransferBuf[KCS_PACKAGE_MAX];
...
{
  {
		(volatile u8 *) LPC_CH_ZERO_REG_DIR,
		(volatile u8 *) LPC_CH_ZERO_REG_DOR,
		(volatile u8 *) LPC_CH_ZERO_REG_CMR,
		(volatile u8 *) LPC_CH_ZERO_REG_STR,
		(volatile u8 *) LPC_CH_ZERO_REG_CTL,
		(volatile u8 *) LPC_CH_ZERO_REG_IC,
		(volatile u8 *) LPC_CH_ZERO_REG_IE
	},
	{
		(volatile u8 *) LPC_CH_ONE_REG_DIR,
		(volatile u8 *) LPC_CH_ONE_REG_DOR,
		(volatile u8 *) LPC_CH_ONE_REG_CMR,
		(volatile u8 *) LPC_CH_ONE_REG_STR,
		(volatile u8 *) LPC_CH_ONE_REG_CTL,
		(volatile u8 *) LPC_CH_ONE_REG_IC,
		(volatile u8 *) LPC_CH_ONE_REG_IE
	},
	{
		(volatile u8 *) LPC_CH_TWO_REG_DIR,
		(volatile u8 *) LPC_CH_TWO_REG_DOR,
		(volatile u8 *) LPC_CH_TWO_REG_CMR,
		(volatile u8 *) LPC_CH_TWO_REG_STR,
		(volatile u8 *) LPC_CH_TWO_REG_CTL,
		(volatile u8 *) LPC_CH_TWO_REG_IC,
		(volatile u8 *) LPC_CH_TWO_REG_IE
	},   
};
...
static void aess_kcs_isr_handler (u8 u8Channel);
        ...
        switch (u8Register)
            ...
            case KCS_REG_FOCTL:
                ...
                if (LPC_RST_INT_ENABLE == u8Data)
                {
                    ...
                    u8Tmp = (*((u8 *)BIOS_PS_REG_FIFO_ENABLE)) | LPC_RST_INT_ENABLE;
                    *((u8 *)BIOS_PS_REG_FIFO_ENABLE) = u8Tmp;
               	}else if (LPC_RST_INT_DISABLE == u8Data)
               	{
                    /* Disable LPC reset interrupt*/
                    u8Tmp = (*((u8 *)BIOS_PS_REG_FIFO_ENABLE)) & LPC_RST_INT_DISABLE;
                    *((u8 *)BIOS_PS_REG_FIFO_ENABLE) = u8Tmp;
                }
                break;
           case BIOS_REG_FIFO_LPCRST_CHGSTS:
                u8Tmp = (*((u8 *)BIOS_PS_REG_FIFO_MISC_STATUS)) | LPC_RST_CHG_STS_CLEAR;
               *((u8 *)BIOS_PS_REG_FIFO_MISC_STATUS) = u8Tmp;
               break;      
           case GC_REG_MFSEL_SMI:
                /*Enable pin function to SMI*/
                //npcmx50_spin_lock_irqsave(&flags);
                u32Tmp = (*((u32 *)GC_REG_MFSEL1)) | ((u32)u8Data << SHIFT_22_BITS);
                *((u32 *)GC_REG_MFSEL1) = u32Tmp;
    ...
    if ((u8Channel < KCS_TOTAL_CHANNEL) || (KCS_CH_NONE == u8Channel)||(KCS_CH_ALL == u8Channel))
    {
        switch (u8Register)
        {
            ...
            case KCS_REG_IDR:
                ...
                break;
            case BIOS_REG_FIFO_LPCRST_CHGSTS:
		            u8Data = ((*((u8 *)BIOS_PS_REG_FIFO_MISC_STATUS))&LPC_RST_CHG_STS) >> SHIFT_6_BITS;
                break;
            case KCS_REG_INT_STS:
                /*If one of KCS0~2, LPC reset interrupts, then return 1*/
                u8Data  = (*((u8 *)LPC_CH_ZERO_REG_STR)& KCS_STATUS_IBF);
                u8Data |= (*((u8 *)LPC_CH_ONE_REG_STR)& KCS_STATUS_IBF); 
                u8Data |= (*((u8 *)LPC_CH_TWO_REG_STR)& KCS_STATUS_IBF); 
                u8Data |= ((*((u8 *)BIOS_PS_REG_FIFO_MISC_STATUS))&LPC_RST_CHG_STS);
                if(u8Data)
                {
...
static void aess_kcs_errorhandler(u8 u8Channel);
