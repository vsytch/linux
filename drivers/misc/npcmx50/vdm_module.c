...
typedef struct my_struct {
        ...
        u8_t *mprxBuffer;
        u8_t BDF_is_set;
        u32_t mprxBufferLength;
        u8_t *mptxBuffer;
        u32_t mptxBufferLength;
...
static u32_t *vdma_buff;
static u32_t *vdma_buff_virt_addr;
static u32_t disable_TX=0;
static u8_t stop_reset_poll=0;
static u8_t vdm_status = VDM_STATUS_CLOSED;
static u8_t reset_detect_poll = 1;
static u8_t reset_detect_poll_is_running = 0;
...
static u32_t vdm_data_for_platform;
 ...
 *
 */
static void bdf_U16_to_bdf_arg(u16_t bdf_U16, bdf_arg_t *apBdf)
{
 ...
 *
 */
static u16_t bdf_arg_to_bdf_U16(bdf_arg_t *apBdf)
{
...
static void _vdm_close(void)
{
	...
	if(VDM_STATUS_OPENED == vdm_status)
	{
		...
		{
			pVDM_Instance = list_entry(pList, vdm_instance_t, list);
			cbInit(&pVDM_Instance->circularBuffer, (pVDM_Instance->mprxBufferLength/sizeof(u32_t)) ,
			            		pVDM_Instance->mprxBuffer , sizeof(u32_t),memcpy,copy_to_user_wrapper);
...
{ 
	long ret=0;
	unsigned long flags;
	u32_t num_of_items_to_read_from_buff =0 ;//, StartPosToRead;
	vdm_instance_t *pVDM_Instance;
	pVDM_Instance=filp->private_data;

	count = count/sizeof(u32_t); // count is in bytes. we need to convert it to words
...
#endif
{
    ...
    switch (cmd)
    {
        ...
        case PCIE_VDM_SET_TRANSMIT_BUFFER_SIZE:
        	...
        	pVDM_Instance->mptxBufferLength=(u32_t)arg;
        ...
        case PCIE_VDM_SET_RECEIVE_BUFFER_SIZE:
        	...
        	pVDM_Instance->mprxBufferLength=(u32_t)arg;
        	pVDM_Instance->mprxBuffer=kmalloc((u32_t)arg,GFP_KERNEL);
...
static void receive_packet_func(u16_t aBDF , u32_t *data , u32_t NumOfWords ,u8_t isThisLastDataInPacket)
{
	u32_t  num_of_items_written;
	struct list_head *pList;
	vdm_instance_t *pVDM_Instance;
...
#if defined CONFIG_ARCH_NPCM750
