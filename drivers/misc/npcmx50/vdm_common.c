...
u32_t vdma_rx_buff_size_int,*vdma_rx_buff,*vdma_rx_buff_virt_addr;

static u8_t ready_for_transmit=0;


extern void nano_delay(u32_t nsec);
...
{
	u32_t regVal = REG_READ(VDMA_ECTL_REG );
...
*
*/
u32_t get_buffer_state(u32_t *pCurrBuffReadPos,u32_t *pCurrBuffWritePos)
{
	u32_t numOfElementsInBuf ;

	*pCurrBuffWritePos =  (REG_READ(VDMA_CDST_REG) - (u32_t)vdma_rx_buff) >> 2; // we are working with u32_t
	*pCurrBuffReadPos =  	(REG_READ(VDMA_ERDPNT_REG) - (u32_t)vdma_rx_buff) >> 2; // we are working with u32_t
 ...
 *
 */
void __vdma_copy_packets_from_buffer(receive_packet_func_t receive_packet_func , u8_t bufferWasOverflowed)
{
	u32_t numOfElementsInBuf ;
	u32_t testReadPos , lengthOfPacketLeft,lengthOfPacket;
	u32_t PCIeHeaderu32;
	u32_t num_of_items_to_read_from_buff ;
	u32_t currBuffReadPos,currBuffWritePos;
	u32_t BDF;
...
//	currBuffWritePos =  (REG_READ(VDMA_CDST_REG) - (u32_t)vdma_rx_buff) >> 2; // we are working with u32_t
//	currBuffReadPos =  	(REG_READ(VDMA_ERDPNT_REG) - (u32_t)vdma_rx_buff) >> 2; // we are working with u32_t
//
...
//	}
	...
	{
		int wasMisReception;
		//PRINTF("<1>vdm: currBuffReadPos = 0x%x \n", (u32_t)currBuffReadPos );

		// check if find next vdm packet
		lengthOfPacket = 0;
		testReadPos=(currBuffReadPos + 2) % vdma_rx_buff_size_int; // third u32 in packet includes vdm number
		PCIeHeaderu32=vdma_rx_buff_virt_addr[testReadPos];
		wasMisReception=0 ;
		while((PCIeHeaderu32 & VDM_VENDOR_ID_MASK_LSB_IN_U32) != VDM_VENDOR_ID_LSB_IN_U32)
		{
			if(0==wasMisReception)
			{
				PRINTF("<1>vdm: rx err addr=0x%x \n", (u32_t)vdma_rx_buff + (currBuffReadPos<<2));
			}
			wasMisReception=1;
//			numOfElementsInBuf--;

			currBuffReadPos = (currBuffReadPos+1) % vdma_rx_buff_size_int;
			REG_WRITE(VDMA_ERDPNT_REG, (currBuffReadPos << 2) + (u32_t)vdma_rx_buff );
...
//		}
		...
		{
			PCIeHeaderu32 = vdma_rx_buff_virt_addr[currBuffReadPos] ;// read first u32 in packet
			if((PCIeHeaderu32 & PCIe_HEADER_FMT_FIELD_MASK) == PCIe_HEADER_FMT_MSG_NO_PAYLOAD)
			{
				lengthOfPacket=4;
			}
			else
			{
				lengthOfPacket =( (PCIeHeaderu32 >> 8) & 0x0300) + (PCIeHeaderu32  >> 24) + 4;// 4 for pcie header length
			}
			...
			else
			{
				...
				{
					BDF=vdma_rx_buff_virt_addr[(currBuffReadPos + 1)  % vdma_rx_buff_size_int ] ;// read second u32 in packet
	...
	{
		vdm_init_common(vdma_rx_buff , vdma_rx_buff_virt_addr , vdma_rx_buff_size_int * sizeof(u32_t) );
	}
}
 ...
 *
 */
int vdm_SendMessage(u8_t route_type, u16_t aBDF,u8_t  *apData,u32_t aLength)
{
	u32_t 	i,txData;
    u8_t     idx;
	u8_t 	tag,paddingBytesNum;
	u8_t 	currLength;
	u32_t pci_message_header[4]={0};
 ...
 *
 */
u8_t vdm_set_timeout(VDM_RX_TIMEOUT_t timeout)
{
	u32_t regVal;
 ...
 *
 */
u8_t vdm_is_ready_for_write(void)
{
 ...
 *
 */
u32_t vdm_get_errors(void)
{
	u32_t regVal = REG_READ(VDM_STAT_REG );
	u32_t retVal = 0;
	//clear flags :
 ...
 *
 */
void vdm_clear_errors(u32_t clear_errors_bitmap)
{
 ...
 *
 */
int vdm_init_common(u32_t *apVdma_rx_buff,u32_t *apVdma_rx_buff_virt_addr,u32_t buff_bytes_len)
{
	u32_t reg_val;
	u8_t buff_size_in_16kb;
	unsigned long flags;
	...
	vdma_rx_buff_size_int   = (buff_size_in_16kb * (16384))/sizeof(u32_t);
	...
	REG_WRITE(VDMA_DSTB_REG,  (u32_t)apVdma_rx_buff );// dst_addr

	REG_WRITE(VDMA_ESRCSZ_REG, PHYS_VDM_STAT_REG_ADDR);	// size_addr
	REG_WRITE(VDMA_ERDPNT_REG, (u32_t)apVdma_rx_buff );		// read pointer
...
{
	u32_t prev_pointer = 0;
	u32_t curr_pointer = 1;
	// stop retrigerring
