				...
				size_t count, loff_t *ppos)
{
	...
	if (copy_to_user((void __user *)buf, (const void *)(mbox->memory + *ppos) , count))
	{
				...
				size_t count, loff_t *ppos)
{
	...
	if (copy_from_user((void *)(mbox->memory + (*ppos)), (void __user *)buf, count))
	{
		spin_unlock_irqrestore(&mbox->lock, flags);
		return -EFAULT;
	}
...
static irqreturn_t npcm7xx_mbox_irq(int irq, void *arg)
{
	...
	/* disable interrupt we'll clear it when we the data gets read */
	regmap_update_bits(mbox->regmap, NPCM7XX_MBOX_BMBXCTL,
		...
		struct platform_device *pdev)
{
	...
	/* clear status register, register is write one to clear */
	regmap_read(mbox->regmap, NPCM7XX_MBOX_BMBXSTAT, &val);
	if (!val) 
		regmap_write(mbox->regmap, NPCM7XX_MBOX_BMBXSTAT, val);
