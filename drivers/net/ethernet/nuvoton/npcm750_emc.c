...
//#define CONFIG_NPCM750_EMC_ETH_DEBUG
...
struct  npcm750_ether {
	...
	unsigned int rdu;
	unsigned int rxov;
	unsigned int rxold_stuck;
	unsigned int rxnew_stuck;
...
static void npcm750_return_default_idle(struct net_device *dev)
{
    ...
    // During the EMC reset the AHB will read 0 from all registers, so in order to see if the 
    // reset finished we can't count on REG_MCMDR.SWR to become 0, instead we read another
...
static void npcm750_reset_mac(struct net_device *dev, int need_free)
{
    ...
    //enable_irq(ether->txirq);
    //enable_irq(ether->rxirq);
...
static int npcm750_clean_tx(struct net_device *dev, bool from_xmit)
{
	struct npcm750_ether *ether = netdev_priv(dev);
...
static irqreturn_t npcm750_rx_interrupt(int irq, void *dev_id)
{
    ...
    {
        u32 RXFSM, TXFSM, FSM1;
        // workaround rxoverrun hung
        RXFSM = __raw_readl(REG_RXFSM);
        TXFSM = __raw_readl(REG_TXFSM);
        FSM1 = __raw_readl(REG_FSM1);
        if (((RXFSM & 0x8000F00F) == 0x80002002) && ((TXFSM & 0x8000F00F) == 0x80002001) && 
            ((FSM1 & 0x0000FF00) == 0x00000000))
        {
            int i;
            for (i=0; i<320; i++)
            {
                ...
                if (((RXFSM & 0x8000F00F) != 0x80002002) || ((TXFSM & 0x8000F00F) != 0x80002001) || 
                    ((FSM1 & 0x0000FF00) != 0x00000000))
                {
                    //dev_err(&pdev->dev, "stuck on REG_RXFSM = 0x%08X REG_TXFSM = 0x%08X REG_FSM1 = 0x%08X status=%08X i=% 3d\n", 
                    //RXFSM, TXFSM, FSM1, status, i);
                    break;
                }
            }
            if (i == 320)
            {
                ether->rxnew_stuck++;
#ifdef CONFIG_NPCM750_EMC_RESET_ON_NEW_ISSUE
                spin_lock_irqsave(&ether->lock, flags);
#ifdef CONFIG_NPCM750_EMC_ETH_DEBUG
				npcm750_info_print(dev);
#endif
                ...
                spin_unlock_irqrestore(&ether->lock, flags);
                //disable_irq(ether->txirq);
                //disable_irq(ether->rxirq);
                ether->need_reset = 1;
                napi_schedule(&ether->napi);
                dev_err(&pdev->dev, "stuck on REG_RXFSM = 0x%08X REG_TXFSM = 0x%08X REG_FSM1 = 0x%08X status=%08X doing reset!\n", 
                    RXFSM, TXFSM, FSM1, status);
                return IRQ_HANDLED;
#endif                
            }
        }
...
static int npcm750_get_settings(struct net_device *dev, struct ethtool_link_ksettings *cmd)
{
	struct npcm750_ether *ether = netdev_priv(dev);
	struct phy_device *phydev = ether->phy_dev;
	...
	pr_info("\n\nnpcm750_get_settings\n");
	phy_ethtool_ksettings_get(phydev, cmd);

	return 0;
}

static int npcm750_set_settings(struct net_device *dev, const struct ethtool_link_ksettings *cmd) {
	struct npcm750_ether *ether = netdev_priv(dev);
	struct phy_device *phydev = ether->phy_dev;
	int ret;
	unsigned long flags;
...
static const struct ethtool_ops npcm750_ether_ethtool_ops = {
	.get_link_ksettings     = npcm750_get_settings,
	.set_link_ksettings     = npcm750_set_settings,
...
static int npcm750_info_dump(char* buf, int count, struct net_device *dev)
{
    ...
    PROC_PRINT("rx_berr       %6d\n", ether->rx_berr);       ether->rx_berr = 0;
    PROC_PRINT("rxold_stuck   %6d\n", ether->rxold_stuck);   ether->rxold_stuck = 0;
    PROC_PRINT("rxnew_stuck   %6d\n", ether->rxnew_stuck);   ether->rxnew_stuck = 0;
...
/* disable for now - need to check if necessary */
 ...
 /*-----------------------------------------------------------------------------------------------------*/
    /* Muxing RMII MDIO                                                                                    */
    /*-----------------------------------------------------------------------------------------------------*/
    if(pdev->id == 0)            // ETH0 - EMC1
    {
	regmap_update_bits(gcr_regmap, MFSEL3_OFFSET, (0x1 << 9), (0x1 << 9));
	regmap_update_bits(gcr_regmap, MFSEL1_OFFSET, (0x1 << 13), (0x1 << 13));
	regmap_update_bits(gcr_regmap, MFSEL1_OFFSET, (0x1 << 12), (0x1 << 12));
	regmap_update_bits(gcr_regmap, INTCR_OFFSET, (0x1 << 5), (0x1 << 5));
    }
    else if(pdev->id == 1)       // ETH1 - EMC2
    {
	regmap_update_bits(gcr_regmap, MFSEL1_OFFSET, (0x1 << 14), (0x1 << 14));
	regmap_update_bits(gcr_regmap, MFSEL1_OFFSET, (0x1 << 16), (0x1 << 16));
	regmap_update_bits(gcr_regmap, MFSEL1_OFFSET, (0x1 << 15), (0x1 << 15));
    }
    ...
    if (pdev->id == 0)
    {
	regmap_update_bits(rst_regmap, IPSRST1_OFFSET, (0x1 << 6), (0x1 << 6));
	regmap_update_bits(rst_regmap, IPSRST1_OFFSET, (0x1 << 6), 0);
    }
    else if (pdev->id == 1)
    {
	regmap_update_bits(rst_regmap, IPSRST1_OFFSET, (0x1 << 21), (0x1 << 21));
	...
	ether_setup(dev);
        ...
        //val = __raw_readl(ether->reg + 0x1000);
	//__raw_writel(val,REG_test2+0x1000);
