...
#ifdef NPCMX50_USB_DESC_PHYS_BASE_ADDR
    udc->ep_qh_dma = (dma_addr_t)NPCMX50_USB_DESC_PHYS_BASE_ADDR(udc->id);
    udc->ep_qh = (void *)NPCMX50_USB_DESC_VIRT_BASE_ADDR(udc->id);
#else
...
static int npcmX50_udc_probe(struct platform_device *pdev)
{
    ...
    // enable devices
	if (npcmx50_udc_enable_devices(udc_controller->id) < 0)
	{		
	...
	*/
...
#ifdef NPCMX50_USB_DESC_PHYS_BASE_ADDR
    {
		...
		MAX_DTD_POOL = ((NPCMX50_UDC_EPQ_DTD_SIZE - size_of_queue_heads) / (2 * DTD_ALIGNMENT));
